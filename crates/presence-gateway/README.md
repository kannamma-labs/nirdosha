...
`docs/WORKFLOW.md`'s own "Deliberate non-goals" section: *"This repository does
...
at-least-once-cleanup limit `docs/WORKFLOW.md`'s own non-goals section already
...
- `cargo test --test gateway_integration` — real, full-stack: a real
  ...
  `crates/compiler/tests/serve.rs::start_server` uses), a real Redis
  (`127.0.0.1:6379` — same instance `crates/compiler/tests/mq.rs` already
  ...
  stand-in" discipline `crates/compiler/tests/mq.rs`'s own doc comment states.

## What's *not* here — stated precisely, not silently implied

- **No TLS termination.** Same posture `nirdosha serve` itself takes
  (`docs/KUBERNETES.md`'s compliance matrix, TLS row: `[N/A]`, "correctly
