...
fn main() -> ExitCode {
    ...
    // (docs/goal.md row 9: structured diagnostics), and scanning it up front
    ...
    // `--transact-log=<path>` (`transact`'s durability log, `docs/TRANSACT.md`'s
    ...
    // (`durability::LogTarget::from_cli_arg`) -- `docs/ROADMAP.md`'s
    ...
    // cluster (`rqlite.rs`) instead of Postgres -- `docs/ROADMAP.md`'s Phase 2,
    ...
    // `docs/WORKFLOW.md`) — same up-front, interpret-only scan as
...
/// only changes how *failures* are reported (docs/goal.md row 9 is about
...
fn cmd_interpret(
    ...
    // `LogTarget::from_cli_arg` (`docs/ROADMAP.md`'s multi-instance fix): a
...
fn cmd_serve(
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            // `docs/WORKFLOW.md`'s presence bridge: the service token an
