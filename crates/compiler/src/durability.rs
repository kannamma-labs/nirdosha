...
//! the default) or a real, shared Postgres database (`docs/ROADMAP.md`'s
...
//! own `txn_id`-keyed idempotency guarantee (`docs/TRANSACT.md`) is silently
...
pub enum LogTarget {
    ...
    /// laziness).
    Postgres(String),
    /// The raw `rqlite://`/`rqlites://` connection string — `docs/ROADMAP.md`
...
impl LogTarget {
    ...
    /// the Raft-replicated-SQLite backend (`crate::rqlite`, `docs/ROADMAP.md`
