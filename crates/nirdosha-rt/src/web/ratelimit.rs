//! A fixed-window, per-process, per-IP rate limiter — ported from
//! `compiled-serve`'s own `ratelimit.rs` (that crate's own doc comment:
//! a fleet-wide limiter is real, separate follow-up work, not silently
//! assumed away; unchanged here). `Router::with_rate_limit` is this
//! module's one caller.
...
/// Hard cap on distinct tracked IPs — without one, a botnet scan or a
/// spoofed source feeding many distinct fake IPs grows `windows`
/// without bound, per-process, forever. Same value and reasoning as
/// `compiled-serve`'s own `MAX_TRACKED_IPS`.
...
mod tests {
    ...
    /// The tracked-IP map must never grow past `MAX_TRACKED_IPS`, and
    /// the entries it keeps after overflowing the cap must be the
