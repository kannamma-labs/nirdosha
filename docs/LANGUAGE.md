...
> `http`/`https`, all of Row 12's identity builtins, and `transact`
> (durability log + crash replay included) are no longer in that
> "interpreter-only" set** — real compiled-path codegen landed for all of
> them (`docs/ROADMAP.md` Track B, `docs/adr/0005`-`0007`, `0009`); only
> `sandbox` remains genuinely interpreter-only-and-thus-not-running.
...
- **`verify`** (2026-09, three-valued verdict added 2026-09) — runs the
  same gates `build` already runs before it ever touches LLVM
  (typecheck, ownership, `validate` contract-check), plus
  `smt::analyze`'s Tier-1 proof-obligation counts, and prints one JSON
  verdict to stdout with a real, **three-valued** exit code: `0` ==
  `PROVED` (every gate passed and every `validate` obligation Z3 could
  model was proved), `1` == `DISPROVED` (a hard pipeline failure, or a
  real Z3 counterexample), `2` == `UNKNOWN` (nothing was disproved, but
  at least one `validate` obligation is outside what Tier 1 models —
  e.g. a non-integer parameter — so Z3 was never asked to decide it).
  Never collapsed to a binary pass/fail: an earlier revision folded
  `UNKNOWN` into a plain `0`/passed exit, which reported confidence this
  pipeline hadn't earned — the top-level JSON `verdict` field
  (`"PROVED"`/`"DISPROVED"`/`"UNKNOWN"`) and the exit code always agree.
  No clang/LLVM toolchain needed, and it never produces a binary. Exists
  so CI and an agent's own repair loop can ask "does this pass?" as a
  single machine-readable call instead of parsing `build`'s stderr text;
  see `crates/compiler/src/main.rs`'s `cmd_verify`/`VerifyVerdict`/
  `ProofVerdict` for the exact schema. Unlike `build`, does not require
  `fn main()` — a tool/library fragment an agent emits under a
  constrained grammar typechecks and verifies the same as a runnable
...
**Database** (`Ty::Db`, compiled 2026-09 — SQLite and Postgres, both pooled; `docs/PROTOLANG_PORT.md`'s "Locked design 5: DB", `docs/adr/0005-postgres-pooling-and-tls.md`)
- `db_connect(conn_str: str) -> Result(db, str)` — a bare path or `:memory:` opens (pooled, except `:memory:` — see below) a local SQLite database (`rusqlite`, `bundled` feature — statically linked, no system `libsqlite3`); a `postgres://`/`postgresql://` connection string opens a real, pooled Postgres connection (`postgres`/`postgres-native-tls`, TLS verify-by-default for any non-`localhost` host unless the connection string already sets its own `sslmode`). Both backends: `kernel::db`, `runtime-kernels/src/kernel/db.rs`.
- **Pooled, not connection-per-call, for both backends except `:memory:`.** `db_connect` checks out from a process-wide pool keyed by the exact connection string (`kernel::pool::PoolRegistry`) rather than opening fresh every time. A pooled connection is validated with a real round-trip (`SELECT 1`, never `is_closed()`-only) on every checkout; a stale one (e.g. after a server restart) is evicted and transparently replaced before the caller ever sees it — a `stale_rehydrated` counter in the kernel flight recorder (`kernel::dump_report`) makes this visible. `:memory:` is the one deliberate exception, never pooled — a `:memory:` SQLite database is private to the connection that created it, so pooling it would silently share (or fragment) state across unrelated callers.
- `db_query(conn: db, sql: str, ...up to 8 bind values) -> Result(json, str)` — row-returning statements (`SELECT`). Bind values are `i64`/`f64`/`str`/`bool`, the only way to parameterize a query since `str` has no concatenation (§2) — a zero-payload `enum` variant as a bind value is the one part of the original design not yet compiled (`NirBindValue`'s own doc comment, `runtime-kernels/src/lib.rs`). On Postgres, a `?` placeholder is rewritten to `$1`/`$2`/... (string-literal- and comment-aware, never touching a `?` inside `'...'` or a `--` comment) and each bind value is encoded according to the server's own reported parameter type (`int2`/`int4`/`int8`, `float4`/`float8`, text) — not a fixed Rust-side width assumption, found necessary by testing against a real server. Every row comes back as one JSON object (column name → value); the whole result set is a JSON array, navigated with `json_array_len`/`json_array_get`/`json_get_*` like any other JSON document. `Ty::Json` compiles as the raw JSON text itself, re-parsed by each accessor — see `json`'s own entry below.
...
- `stop(conn)` — closes the connection (reuses `tcp`/`file`'s keyword) — for a pooled connection, this returns it to the pool for reuse rather than closing the underlying socket/file handle.
- A connection failure, SQL syntax error, or constraint violation is `Err(message)`, never a trap — the backend's own error message (`rusqlite::Error`/`postgres::Error`) passed straight through.
...
- SQLite `BLOB` columns have no first-class Nirdosha type to carry them (no `bytes` type) — `db_query` represents one as JSON `null`, a disclosed, narrower cut, not a silent drop; every other SQLite storage class round-trips exactly. A Postgres column type this backend doesn't specifically recognize is likewise read back as JSON `null`, not an error.
- Real, compiled-and-run verification: `examples/features/27_database.nir` (schema creation, parameterized insert/update, a filtered `SELECT`, a connection failure) — `crates/compiler/tests/codegen.rs`'s `db_connect_execute_query_round_trips_real_sqlite_rows` compiles and runs that exact file, unmodified, against a real in-memory SQLite database. Postgres: `crates/compiler/tests/postgres.rs` (real, opt-in, `NIRDOSHA_TEST_POSTGRES_URL`-gated, `#[ignore]` by default — `docker-compose.dev.yml` at the repo root stands up a real local server), plus `runtime-kernels`' own `kernel::db::tests`/`db_kernel_tests` (pooling identity, rehydration-after-a-killed-backend, placeholder rewriting).
...
**HTTP/HTTPS client** (no new `Ty` — one call, one response, never a language-level persisted connection handle, even though a real keep-alive connection is pooled underneath, entirely transparently; `HttpResponse { status: i64, body: str }` is a plain prelude struct; compiled 2026-09, real pooling + admission control 2026-09 — `docs/adr/0006-http-keepalive-pooling.md`)
- `http_get(host: str, port: i64, path: str) -> Result(HttpResponse, str)` / `http_post(host, port, path, body: str) -> Result(HttpResponse, str)` — plain HTTP, pooled real keep-alive connections (`kernel::http::HttpManager`).
- `https_get`/`https_post` — same request/response handling, pooled over a `native_tls::TlsStream` (vendored OpenSSL — see below). `TlsConnector::new()`'s defaults do the actual security-critical work (certificate-chain and hostname verification against the platform's trust store).
- **Real HTTP/1.1 keep-alive, not `Connection: close`.** Every request sends `Connection: keep-alive`; the response is read by a real, incremental `Content-Length`/chunked-aware parser (`kernel::http::read_http_response`) that stops exactly at the end of one response — never reading to EOF, which cannot work once the peer might not close the connection. A response that itself declares `Connection: close` is honored: that connection is evicted, never reused for a later call. A `Transfer-Encoding: chunked` response body is decoded the same way either way (found necessary by testing against a real server, not designed in advance — a large fraction of real HTTP servers chunk by default).
- **Pooled**, one `PoolRegistry` per transport, keyed by `host:port` — a call reuses an already-open, still-good connection instead of paying a fresh TCP/TLS handshake every time. Plain HTTP validates a pooled connection with a real non-blocking peek before reuse; HTTPS has no safe way to peek through an active TLS session, so a stale HTTPS connection's staleness surfaces as a real send failure instead — handled by an **at-most-once retry**: a failure while *writing* the request (provably never reached the server) retries once against a fresh connection; a failure while *reading* the response is never retried, since the server may already have processed a non-idempotent `POST`.
- Real admission control: `Domain::Http` (previously nonexistent — `http_get`/`post` had zero admission control before this), held for the duration of one request (both attempts, if a retry happens).
...
| `transact` | Yes | 2026-09. `precheck?/network/verify/commit/compensate?/log?`, real control flow, a real `bool` result. `network`'s `retry`/`timeout` rejected explicitly (architectural: a compiled trap is `abort()`, unrecoverable, and `network`'s return type can never be `Result(_, _)` — no failure signal to retry on). **2026-09 follow-up**: durability log (fsync'd SQLite), bounded retry-with-backoff for `commit`/`compensate` when their return type is `Result(_, _)`, and compiler-synthesized crash replay from generated `main`'s prologue — see `docs/adr/0009-transact-durability-and-replay.md`. Disclosed, not fixed here: no build-version fingerprint guard on replay dispatch, and the durability log is local-SQLite only (unsafe under a multi-replica deployment). |
