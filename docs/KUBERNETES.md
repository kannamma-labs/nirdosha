...
protobox's use of it, see `docs/PROTOBOX_INTEGRATION.md`) needs to be a fully
...
from `docs/ROADMAP.md` Track A2 ("Deployment story for the interpreted path")
and `docs/PROTOBOX_INTEGRATION.md` §7/§9 — this file is the detail those
...
just what's missing today — see `docs/KUBERNETES_ADVANTAGE.md`.

Same discipline as `docs/ROADMAP.md`'s own standards table: every
...
|---|---|---|---|
| OCI-compliant image | `[DONE]`, verified live | Repo-root `Dockerfile` (multi-stage: `rust:1-slim-trixie` build stage with the `dist` feature — z3-src 416.0.2 needs libstdc++'s `<format>`, which requires Debian trixie/GCC≥13, confirmed by a real build failure against bookworm/GCC 12 first; `python:3.12-slim-trixie` runtime stage, matching the build stage's Debian release exactly — a first version mismatched them (bookworm runtime), and the built binary genuinely failed to start with `GLIBC_2.39'/`GLIBCXX_3.4.31' not found, confirmed by a real `docker run`, not assumed — with `pytest`/`requests` baked in for protobox's black-box QA tasks). Verified fully live, not just built: `docker run nirdosha serve` inside the built image actually boots, `curl` against `/healthz`/`/readyz`/`/metrics`/`POST /api/ping` from the host all return correct real responses, `docker stop` (SIGTERM) exits cleanly (code 0) in ~0.4s, and `id` inside the container confirms UID 10001 non-root. | Still needs an actual `docker push` to `ghcr.io/protobox/nirdosha-runtime` from a real CI run with registry credentials — `.github/workflows/docker.yml` (added, lints clean under `actionlint`) does this on every `v*` tag; not yet exercised against the real registry from this change. |
...
| `workflow`/`transact` durability across replicas | `[DONE]` | `docs/ROADMAP.md` Track A17, `durability.rs`: `--transact-log`/`--workflow-log` accept a `postgres://`/`postgresql://` value (`LogTarget`, `pool.rs`'s `PoolRegistry`). Verified live against a real Postgres server: two independent `WorkflowLog`/`TransactLog` handles sharing one database never collide on `instance_id` and do see each other's `txn_id` rows; two real `serve` processes on the same database both serve concurrent requests correctly. | Protobox deploy config: point both flags at Postgres for any project that will run >1 replica. No further runtime work needed. |
| Business/CRUD data (`db_connect`) on shared storage | `[PARTIAL]` | `dbconn.rs`: `DbConn::{Postgres, PostgresTls}`, pooled (`POSTGRES_POOLS`), TLS-optional — the runtime capability is real and shipped. protobox's `gen-crud`/`nirdosha_screen_plan.py` always emit a local `<project_id>.db` literal today (see `docs/PROTOBOX_INTEGRATION.md` §8's first gotcha for the related `db_connect` vs `--db` mismatch trap). | Protobox-side codegen change only — no nirdosha runtime work needed. |
...
| Cross-machine SQLite replication (no Postgres) | `[N/A]` | `docs/ROADMAP.md` Track A17 Phase 2 names rqlite/dqlite (Raft) and cr-sqlite (CRDT) as real prior art, deliberately not adopted — Postgres already solves the actual need. | Revisit only if a deployment specifically must avoid a Postgres dependency. |
...
| mTLS between services | `[OPEN]` | Confirmed absent repo-wide; also the FAPI blocker per `crates/compiler/UI_DSL_TODO.md:353-357`. | Delegate to a service-mesh sidecar (Istio/Linkerd) rather than building it into nirdosha. |
| Rate limiting | `[OPEN]` | Zero matches in `serve.rs` — already named as a real gap in `docs/ROADMAP.md`'s OWASP Top 10 compliance row. | Push to an Ingress annotation/mesh policy as the near-term mitigation. |
...
| Helm chart / Kustomize overlay | `[DONE]` | `deploy/helm/nirdosha/` (full chart: StatefulSet/Deployment, Service, Ingress, ServiceAccount, PodDisruptionBudget, `_helpers.tpl`/`_pod.tpl` shared templates, `NOTES.txt`) — `helm lint` clean, `helm template` verified for the sqlite default, a postgres+3-replica+ingress+auth override combination, and the sqlite-with->1-replica combination correctly `fail`s at render time. `deploy/kustomize/base/` (static sqlite render) and `deploy/kustomize/overlays/postgres-multi-replica/` (static postgres render) — both verified with `kustomize build`. protobox's own `plugins/deploy_targets/kubernetes.py` (new deploy target, registered the same auto-discovery way `neon.py`/`render.py` are) renders the identical shape programmatically and additionally builds+pushes a per-project derived image (`_derived_dockerfile`: the base runtime image + that project's `.nir`/`theme.json` layered on top, reusing `_render_api.build_and_push_image`, the same GHCR-push helper `render.py` already uses) before applying — 29 tests in `tests/plugins/deploy_targets/test_kubernetes.py`. | `is_configured()` (a `KUBE_CONTEXT` env var + `kubectl` on `PATH`) gates the live-`kubectl apply` half only — manifests are always written to the workspace even without a live cluster, so an operator can apply them by hand. Not yet exercised against a real live cluster from this change (only `helm template`/`kustomize build`/mocked-`kubectl` paths were verified). |
| Presence-gateway sidecar (`notify()`'s real-time push, docs/ROADMAP.md A5) | `[DONE]` | **2026-08-28.** Chart bumped to `0.2.0`: `presence.enabled: true` now deploys `presence-gateway/`'s own image as a second container in the same Pod (`_pod.tpl`), reusing the main container's `auth.jwksSecretName`/`issuer`/`audience` and `presence.tokenSecretName` (no new Secret to create), plus new `presence.gateway.*`/`presence.redis.*` values; the gateway's WS port is exposed on the same Service. `presence.enabled` without `auth.enabled` or without `presence.redis.host` set now `fail`s at render time (`_helpers.tpl::nirdosha.validatePresence`) — the same posture `validateReplicaMode` already takes — rather than the silent trap this row used to describe: before this, `presence.enabled: true` alone wired `--presence-token-file` onto the main container (stopping the routes from 404ing) but deployed no actual gateway, so `notify()` still always silently took the offline path, indistinguishable from `presence.enabled: false` except for a token file nobody read. Verified live, not just rendered: a Docker image built from `crates/presence-gateway/Dockerfile`, run with the *exact* `securityContext` this chart applies (arbitrary non-owning UID 10001, `readOnlyRootFilesystem: true`, no `/data` mount) against a real `nirdosha serve` + real Redis + a real browser-shaped `WebSocket` client, round-trips a real `notify()` call correctly. `deploy/kustomize/`'s two static renders regenerated from the bumped chart (`helm template`, per each file's own "regenerate, don't hand-edit" comment) — diff was the `helm.sh/chart` label only, confirming no unrelated drift; no dedicated Kustomize overlay added for presence specifically, consistent with `auth`/`otel` also having none (Kustomize captures the two replica-mode archetypes only, every optional feature toggle is a Helm-chart-only knob). `.github/workflows/docker.yml` matrixed (same job, two images) to also multi-arch-build+sign+SBOM `ghcr.io/protobox/nirdosha-presence-gateway` on the same `v*`-tag trigger — `actionlint` clean, not yet run for real (same as the runtime image's own row above). | Same as the runtime image above: neither image has actually been pushed to the real registry from a live tag push yet. |
...
`crates/compiler/src/{main,serve,dbconn,durability,instance_lock,
observability,pool}.rs`, `docs/ROADMAP.md` (Track A items A1/A2/A17, the
Standards & compliance matrix section), `.github/workflows/{build,
release}.yml`, `docs/PROTOBOX_INTEGRATION.md` §§7-9, and protobox's
