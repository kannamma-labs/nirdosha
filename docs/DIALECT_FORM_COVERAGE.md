...
| `workflow Name { .. }` | `WorkflowDecl` (`ast.rs:1699`), lowered by `workflow_lower.rs` into ordinary `fn`s/`enum`s | `wizard!` (commit `9a78fa5`) | **Partially covered.** `wizard!` is one archetype (a multi-step form with server-side per-run state) built on the same states/transitions idea `WorkflowDecl` has, not a general raw-workflow grammar substitute for every shape `WorkflowDecl` accepts. A generic `workflow! { state ... on <event> -> <target> ... }` mirroring `WorkflowDecl` directly is not implemented. |
...
2. **A general `workflow!`** — `wizard!` covers one shape; a raw state/transition
   grammar mirroring `WorkflowDecl` directly would let a workflow that doesn't fit
   the multi-step-form pattern still avoid hand-rolled state machinery.
