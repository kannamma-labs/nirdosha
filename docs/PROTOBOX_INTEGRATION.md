...
`RoleMapping` boilerplate itself (`docs/ROADMAP.md` Track A6).
...
(`docs/GRAMMAR.md` is the ground truth if anything here and the grammar ever
disagree):
...
Full `serve` flag reference (`crates/compiler/src/main.rs::cmd_serve`):
...
| `--presence-token` | `docs/WORKFLOW.md`'s live-push bridge, if used |
...
DB and durability logs are pointed at Postgres) — see `docs/KUBERNETES.md`.
...
## 9. Open gaps (tracked in `docs/ROADMAP.md`)
...
- **Single fixed IdP per running server** (`docs/ROADMAP.md` Track A6, "Multi-
  IdP registry") — `serve` takes exactly one `--jwks-file`/`--issuer`/
  `--audience` triple; a protobox project needing multiple trusted IdPs
  isn't supported yet.
- **Containerized/production deployment story** (`docs/ROADMAP.md` Track A2,
  ...
  verified (`docs/KUBERNETES.md`'s P0/P2/P3 remediation items). Still
  ...
  Full detail in `docs/KUBERNETES.md`.
- **Business-rule parameters (thresholds, boundary operators, currency)
  have no elicitation or config-store path** (`docs/ROADMAP.md` Track A9) —
...
- **`_Stack.prompt_rules()` (`plugins/languages/nirdosha.py`) — the
  ...
  (`docs/LANGUAGE.md` §6b — free text crossing a `fn` boundary must wrap in
  `struct Text { value: str }`), field-level `pattern`/`format`/`min`/
  `max` validation (`docs/LANGUAGE.md` §11), or the fixed chart/animation/
  form-control sets (`crates/compiler/UI_DSL_TODO.md`'s "Deliberate non-goals",
...
- **`--theme`/`DesignSpec` integration is real, shipped, and load-bearing
  ...
  specifically as a 1:1 mirror of that function's JSON shape (`docs/LANGUAGE.md`
...
- `scratch/nirdosha_llm_prompt.md` — the generation prompt (§3), tracked in
  this repo.
- `crates/compiler/src/init.rs`, `crates/compiler/src/main.rs::cmd_init` — §2.
- `crates/compiler/src/serve.rs`, `crates/compiler/src/main.rs::cmd_serve` — §5.
- `crates/compiler/src/migrate.rs` — §5's migration behavior.
- `docs/LANGUAGE.md` §6b (str-ban/`Text`), §11a (role mapping), §13 (migrations).
- `docs/WORKFLOW.md` — provider-config/notification mechanics behind
  `EmailProviderConfig` et al.
- `docs/ROADMAP.md` Track A (A2, A6, A9) — §9's open gaps, in full detail.
- `crates/compiler/tests/trade_finance_governance_routing.rs` — the threshold/
  boundary-operator drift cited in §9's A9 item, verified against the
  real `.nir` routing rule rather than asserted from prose.
- `docs/MOBILE.md` — the `target:` key proposal referenced in §10's last item.
- `crates/compiler/UI_DSL_TODO.md` — the "Deliberate non-goals" section §10
