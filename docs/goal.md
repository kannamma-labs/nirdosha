...
|---|---|---|---|---|---|
| 1 | No GC, no manual `free()` | **Hard** — proof | Ownership / linear types + region inference | Rust, Austral, ATS | Cyclic structures (graphs, doubly-linked lists) fight the discipline — needs an arena escape hatch. Delivered today for `box`-typed values via affine tracking; `&` references are typechecked but have no `&mut`-style liveness/exclusivity enforcement yet beyond "the referent isn't already moved" (`ownership.rs`'s own doc comment) — the region-inference half of this row is not fully built |
