...
Want the verdict as data instead of reading terminal output? `nirdosha verify <file.nir>` runs the same typecheck/ownership/contract-check gates `build` does — no LLVM/clang toolchain, no binary produced — and prints a JSON verdict with a real exit code, built exactly for a CI pipeline or an agent's own repair loop to call. See `docs/LANGUAGE.md` §1.
