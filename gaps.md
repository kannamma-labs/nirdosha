...
| TLS termination for `nirdosha serve` itself | Old compiler's `serve.rs` used plain HTTP; doc says production needs reverse proxy. | 2026-09-17: `web::Router::with_tls` (`web/tls.rs`, real `rustls`/`tokio-rustls`, both `Runtime::Sync`/`Async`). A deployer's reverse proxy remains a real, still-supported option (`with_tls` never called, unchanged default). Proven end-to-end over real sockets, both transports, with a real client that validates the server's cert (`crates/nirdosha-rt/tests/tls.rs`) and a negative test that a plaintext client can't talk to a TLS-terminated server. | `IMPLEMENTED` | See `rfcs/0022-web-layer-hardening-and-switchable-transport.md`'s own update section for the design, and its own list (DPoP, fleet-wide rate limiting, security headers) for what shipped alongside it the same day. |
