...
| `requires(claim = "...", "...")` / claim proofs | Doc form grammar supports `claim`; `.nir` had `ClaimView`. | `nirdosha-rt` has no claims concept at all. `nirdosha-macros` only handles `role`. `nirdosha-contract-core/src/model.rs` models both, but macro expansion never generates claim proofs. | `MISSING` / `ACHIEVABLE VIA RUSTC/MACRO/DRIVER` | Can be added as a sibling to `RoleProof` with a `ClaimProof` type and macro injection. No new runtime service needed. |
...
| Claims-based identity (`extract_claim`, `check_claim`) | `.nir` Row-12 had `ClaimView`; dialect doc mentions claim keys. | Not present in `nirdosha-rt/src/role.rs` or macros. | `MISSING` / `ACHIEVABLE VIA RUSTC/MACRO/DRIVER` | Add `ClaimProof` type + macro injection; no new runtime kernel. |
