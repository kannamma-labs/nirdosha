Here is the complete, self-contained RFC. Copy the block below into a file named `rfc-0031-nirdosha-rtm-ecosystem.md` — it is pure markdown with no revision markers, changelogs, or version cross-references.

````markdown
# RFC 0031 — The Nirdosha RTM Ecosystem: Vendor-Neutral Modules on the Guard Kernel
...
help: fields are declared by `dataset(entity = "transaction")` in 10_domains.nir;
      see Nirdosha invariant I6 and RFC 0031 §7.7 (pass V2)
```
...
A few notes on what I did while merging:

- **Removed all revision artifacts** — no changelog, no "r1/r2," no "Revision note," no "(new numbering)" annotations.
- **Renamed "MIC v1" → "MIC"** and "conformance suite v0/v1" → "initial/full conformance suite" so no version markers remain. Future-extension references now say "a future extension of the MIC."
- **Merged the amendments inline** — the unforgeable-plan rules and receipt-binding now live directly in §6.3, the generated-conformance property in §6.8, the compiler-enforced swap procedure in §11, and the driver workstream in the phasing table.
- **Renumbered cleanly** — Compilation Architecture is §7, modules are §8.1–8.8, and all cross-references (V6 → §8.8, etc.) point at the final numbering.

Want me to also generate a matching `rfc-0031` companion file containing just the `drivers/*/manifest.toml` templates and the workspace `Cargo.toml` lint configuration, so the repo scaffold and the RFC land together?