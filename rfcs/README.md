...
| [0022.b](./0022-screen-map-app-shell-login.md) | The hi console's screen-only graph — `login!`/`app_shell!` archetypes and a header toggle between the code datastructure graph and the screens interconnections map, with navigation edges reverse-engineered from each app's own code (nav register, redirects, inbox detail paths) — §2–§5 shipped (§2 in its 2026-09-22 amendment form) |
