...
//! discipline `compiler/tests/mq.rs`'s own doc comment states: a real
//! `nirdosha serve` (in-process, `nirdosha::serve::run`, the same
//! technique `compiler/tests/serve.rs::start_server` uses), a real Redis
//! (`127.0.0.1:6379` — the same instance `compiler/tests/mq.rs` already
...
// Same symmetric test JWKS `compiler/tests/serve.rs`'s own `mint_token`
...
/// same technique `compiler/tests/serve.rs::start_server` uses) with the
...
/// minter `compiler/tests/serve.rs::mint_token` drives (`LANGUAGE.md`
/// §9).
