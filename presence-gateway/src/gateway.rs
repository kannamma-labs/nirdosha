...
//! `WORKFLOW.md`'s presence bridge names this shape directly: "a small
    ...
    if drained.is_err() {
        eprintln!(
            "graceful drain timed out after {:?} with {} connection(s) still open — exiting anyway \
             ...
             the same disclosed at-least-once-cleanup limit WORKFLOW.md's own non-goals section already \
