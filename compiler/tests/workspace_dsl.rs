//! Tests for `ROADMAP.md` Track E1's `workspace`/`panel` DSL — a
//! composite, multi-panel screen scoped to one instance of a `subject`
//! struct. See `GRAMMAR.md`'s `workspace_decl`/`panel_decl` productions
...
// `str` is banned as a function param/return type everywhere (LANGUAGE.md
