//! `workflow { ... }`'s durable state store (`WORKFLOW.md`). Backed by
...
//! `ROADMAP.md`'s multi-instance coordination fix); see that module's own
...
//! replicas. Owns everything `WORKFLOW.md`'s desugared
...
/// (`instance_lock.rs`, `ROADMAP.md`) is a *process*-level concern, not a
...
impl WorkflowLog {
    ...
    /// `ROADMAP.md` Phase 2: a real, Raft-replicated SQLite cluster
...
// --- rqlite implementations, one per method (`ROADMAP.md` Phase 2) ---
