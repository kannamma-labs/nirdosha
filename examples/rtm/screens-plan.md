...
| 2026-09-22 | Nav-reachability repair, pass 1 (screens already marked `built`/`interim` whose real routes 404'd or 403'd — found by logging in as all 11 demo roles and curling every rendered nav link, not by inspection): (a) **route-string fixes** in `menus.toml` — 2.3 `/team`→`/dashboard/team`, 2.4 `/mlro`→`/dashboard/mlro`, 10.1 `/screening`→`/screening/hits`, 8.1 `/rules`→`/rules/windows`, 20.1 `/audit/log`→`/audit/chains`, 20.2 `/audit/changes`→`/audit/config-history`, 20.3 `/audit/packs`→`/audit/pack/export`, 12.11 `/sar/controls`→`/sar/tipping-off`; (b) **serve.nir mount-ordering bugs** (literal path registered after a sibling `{id}` wildcard that swallowed it — same class B8/C2 already found and fixed elsewhere): `mount_sla_monitor` (3.9) moved before `mount_alert_screens`'s `/alerts/{id}`; `mount_continuing_activity_sar` (12.9) moved before `mount_sar_screens`'s `/sar/{id}`; (c) **missing `guard_policy!` grants** for roles `screens.toml` already declared but no policy ever backed — `analyst-read-alert` (3.1) gained ComplianceLead/QaReviewer; `lead-sla-aggregate` (3.9) gained Mlro; `case-read` (4.1) gained ComplianceLead; `analyst-search-transaction` (6.1) gained OpsAnalyst; `ops-read-holds` (11.1/2.5) gained ComplianceLead — closing the `m07_m11_m12_m17_screens.rs` test's own previously-disclosed "ComplianceLead has no plain read grant on payment" gap, now closed for real, test updated to assert 200; new `rules-window-read` (8.1, `75_lineage.nir`) — "window" had NO read policy at all; new `analyst-read-screening-hit` (10.1, `40_scoring.nir`) — "screening_hit" had NO read policy at all; new `qa-review-queue-read` (14.1, `95_qa.nir`) — QaReviewer had create/update on `qa_review` but never read, so its own queue 403'd; new `sar-workbench-read` (12.1, `65_sar.nir`) for Analyst/Mlro/ComplianceLead — `sar_bundle` had create/update/export policies but no plain read at all; (d) **`approval_inbox!` macro gained real multi-role access** (`crates/nirdosha-macros/src/approval_inbox.rs`): `access: requires role "X" or role "Y"` — was single-role-only (`get_gated::<R>`), so `/approvals` (8.8) hardcoded to ComplianceLead alone even though screens.toml lists Mlro too; multi-role case emits a dead-type-alias existence assertion per role (same T-10 mechanism `app_shell_from_toml!` uses) plus a runtime `auth.has_role` OR-check, falling back to `Response::forbidden()`; (e) **two new lookup landing pages** for id-keyed detail-only routes that can't be linked directly from nav (`m21_restricted.nir`): `mount_cs_lookup` (`/cs/lookup`, was `/cs/payments/{id}` with no `{id}`-free entry point) and `mount_rm_lookup` (`/rm/customers`, was `/rm/customers/{id}` same gap) — plain id-entry forms matching `cs-payment-status`/`rm-restriction-view`'s own `cap(row_cap = 1)` shape, not a list. All fixes are additive grants/route corrections, not scope changes — no screen's `stage` moved (all were already `built`; they are now honestly built). 3 pre-existing tests asserting the old, disclosed gaps updated to assert the new, intended-and-real behavior (`guarded_ingest.rs`'s OpsAnalyst subject-list, `m07_m11_m12_m17_screens.rs`'s wall/ComplianceLead 200, `m08_m09_m10_m13_screens.rs`'s screening-hit-list and window-read 200s — `refdata` read intentionally left 403, untouched resource). Still broken after this pass (Class 3 — genuinely never mounted, tracked for pass 2): `/home` `/handover` `/devices` `/network` `/network/shared-attributes` `/transactions/counterparties` `/transactions/structuring` `/qa/scorecards` `/rules/performance` `/intervention/outcomes` `/sar/calendar` `/reports` `/reports/scheduled` `/reports/trends` `/screening/config` `/screening/lists` `/screening/campaigns` `/screening/rescreen` `/ops/*` (7 routes) `/admin/*` (9 routes, one of which — `/admin/sla`→real path is `/admin/sla-config`, still to fix) | `cargo test -p rtm` all 15 suites green after 3 test updates; manual curl audit (all 11 demo roles × every rendered nav link) confirms zero remaining 404/403 among previously-`built` screens |
